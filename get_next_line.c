...
/*   Updated: 2022/05/23 11:20:04 by lperrin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	/* Va prendre la premiere et seconde ligne dans prev_next_str */
	if (!pre_next_str)
		return (NULL);
	line = ft_get_line(pre_next_str);
	/* Va trouver la ligne correspondante dans pre_next_str */
	pre_next_str = ft_new_next_str(pre_next_str);
	/* Va changer la valeur de static char 
	pre_next_str pour pouvoir relancer le programme */
	return (line);
}
